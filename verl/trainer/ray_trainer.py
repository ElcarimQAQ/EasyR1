...
# ###
#     def _create_dataloader(self) -> None:
#         # 初始化 ActionTokenizer（参考 finetune.py 和 train.py）
#         self.action_tokenizer = ActionTokenizer(self.tokenizer)

#         # 定义 RLDS 数据预处理（参考 finetune.py 的 RLDSBatchTransform）
#         batch_transform = RLDSBatchTransform(
#             action_tokenizer=self.action_tokenizer,
#             # tokenizer=self.tokenizer,
#             base_tokenizer=self.processor.tokenizer,  # 传递 tokenizer
#             image_transform=self.processor.image_processor.apply_transform,
#             prompt_builder_fn=PurePromptBuilder,  # OpenVLA 默认使用 PurePromptBuilder
#             use_proprio=False,  # 明确禁用
#         )

#         # # 加载训练数据集（参考 finetune.py 和 train.py 的 RLDSDataset）
#         # self.train_dataset = RLDSDataset(
#         #     data_root_dir=self.config.data.data_root_dir,  # datasets/open-x-embodiment
#         #     dataset_name=self.config.data.dataset_name,    # openx/bridge_v2
#         #     batch_transform=batch_transform,
#         #     resize_resolution=(224, 224),                  # OpenVLA 默认 224x224
#         #     shuffle_buffer_size=100_000,                   # 参考 finetune.py
#         #     image_aug=False,                               # PPO 不需增强
#         # )

#         # # 保存数据集统计信息（用于动作解码，参考 finetune.py 和 train.py）
#         # run_dir = "runs/ppo_openvla"  # 自定义运行目录
#         # if not os.path.exists(run_dir):
#         #     os.makedirs(run_dir, exist_ok=True)
#         # if hasattr(self, 'overwatch') and self.overwatch.is_rank_zero():
#         #     save_dataset_statistics(self.train_dataset.dataset_statistics, run_dir)
#         # else:
#         #     # 如果没有 overwatch 属性，直接保存（假设 rank 0）
#         #     save_dataset_statistics(self.train_dataset.dataset_statistics, run_dir)

#         # # 加载验证数据集
#         # self.val_dataset = RLDSDataset(
#         #     data_root_dir=self.config.data.data_root_dir,
#         #     dataset_name=self.config.data.data_val_name,
#         #     batch_transform=batch_transform,
...
#         # data_path = os.path.join(self.config.data_root_dir, self.config.dataset_name)
#         # self.train_dataset = RLDSDataset(
#         #     data_path=data_path,  # 替换为 data_path
#         #     batch_transform=batch_transform,
#         #     resize_resolution=(224, 224),
#         #     shuffle_buffer_size=100_000,
#         #     image_aug=False,
...
#         # # 同样处理 val_dataset
#         # val_data_path = os.path.join(self.config.data_root_dir, self.config.data_val_name)
#         # self.val_dataset = RLDSDataset(
...
#         data_root_dir = self.config.data.data_root_dir  # /workspace
#         data_mix = self.config.data.data_mix            # bridge_dataset/1.0.0

#         print("Starting to load RLDSDataset...")
...
#             image_aug=False,
#         )
#         print("RLDSDataset loaded successfully.")
#         # 检查数据集大小
#         dataset_size = len(self.train_dataset)
#         if dataset_size <= 0:
#             raise ValueError(f"Dataset {self.config.dataset_name} is empty, size: {dataset_size}")
...
#         # 定义 Collator（参考 finetune.py 的 PaddedCollatorForActionPrediction）
...
#         self.train_dataloader = StatefulDataLoader(
#             dataset=self.train_dataset,
#             batch_size=self.config.data.rollout_batch_size,  # 64
#             # sampler=sampler,
#             num_workers=0,                                  # RLDS 要求 0
#             collate_fn=collator,
#             pin_memory=False,
#             drop_last=True,
#         )
...
#                 print(f"Batch dict before modification: {batch_dict}")
#                 if "proprio" in batch_dict:
#                     del batch_dict["proprio"]
#                 if "dataset_names" in batch_dict:
#                     del batch_dict["dataset_names"]
#                 print(f"Batch dict after modification: {batch_dict}")
...
#                 print(f"TensorDict batch: {batch.keys()}")

#                 # 提取生成所需字段（移除 position_ids 的强制要求）
#                 gen_batch = batch.pop(
#                     ("pixel_values", "input_ids", "attention_mask"),
#                     default=None
#                 )
#                 if gen_batch is None:
#                     raise ValueError("Required fields for generation not found in batch")

#                 train_batch = batch  # 剩下的字段（labels, actions 等）
...
from . import core_algos
from .config import PPOConfig
from .metrics import compute_data_metrics, compute_throughout_metrics, reduce_metrics
...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.KLController, kl_penalty="kl"):
    ...
    if "ref_log_prob" in data.batch.keys():
        kld = core_algos.kl_penalty(
            data.batch["old_log probs"], data.batch["ref_log_prob"], kl_penalty=kl_penalty
        )
        kld = kld * response_mask
        beta = kl_ctrl.value
    else:
    ...
    token_level_rewards = token_level_scores - beta * kld
    current_kl = VF.masked_mean(kld, mask=response_mask, axis=-1)
    current_kl = torch.mean(current_kl, dim=0).item()
...
class RayPPOTrainer:
    def __init__(
        ...
        self.hybrid_engine = config.worker.hybrid_engine
        if not self.hybrid_engine or Role.ActorRollout not in role_worker_mapping:
            raise NotImplementedError("Hybrid engine with ActorRollout role is required.")
        ...
        self.kl_ctrl = core_algos.get_kl_controller(config.algorithm) if self.use_reference_policy else core_algos.FixedKLController(kl_coef=0.0)
    ...
    def _create_dataloader(self) -> None:
        ...
        self.train_dataset = RLDSDataset(
            data_root_dir=self.config.data.data_root_dir,
            data_mix=self.config.data.data_mix,
            batch_transform=batch_transform,
            resize_resolution=(224, 224),
            shuffle_buffer_size=100_000,
            train=True,
        ...
        # print(f"Sample instruction: {sample['instruction']}, image: {sample['image']}")
        ...
        for _ in range(self.config.trainer.total_episodes):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch = batch.select(
                    batch_keys=["input_ids", "attention_mask", "pixel_values"],
                    non_tensor_batch_keys=[],
                )

                # 设置 temperature
                gen_batch.meta_info["temperature"] = self.config.worker.rollout.temperature
                ...
                if self.config.algorithm.adv_estimator == "remax":
                    gen_baseline_batch = deepcopy(gen_batch)
                    gen_baseline_batch.meta_info["do_sample"] = False
                    gen_baseline_batch.meta_info["temperature"] = self.config.worker.rollout.temperature
                    gen_baseline_output = self.actor_rollout_wg.generate_sequences(gen_baseline_batch)
                    batch = batch.union(gen_baseline_output)
                ...
                if self.use_reference_policy:
                    batch.meta_info["temperature"] = self.config.worker.rollout.temperature
                    ref_log_prob = self.ref_policy_wg.compute_ref_log_prob(batch)
                    batch.batch["ref_log_prob"] = ref_log_prob

                reward_tensor = self.reward_fn(batch)
                batch.batch["token_level_scores"] = reward_tensor

                if not self.config.worker.actor.use_kl_loss:
                    batch, kl_metrics = apply_kl_penalty(
                        batch, kl_ctrl=self.kl_ctrl, kl_penalty=self.config.algorithm.kl_penalty
                    )
                    metrics.update(kl_metrics)
                ...
                # 将 advantages 放入 batch 中
                batch.batch["advantages"] = reward_tensor

                # 确保 temperature 在 batch.meta_info 中
                batch.meta_info["temperature"] = self.config.worker.rollout.temperature

                if self.use_critic and self.config.trainer.critic_warmup <= self.global_step:
                    critic_output = self.critic_wg.update_critic(batch)
                    critic_output_metrics = reduce_metrics(critic_output.meta_info["metrics"])
                    metrics.update(critic_output_metrics)
                ...
                if self.val_reward_fn is not None and self.config.trainer.val_freq > 0 and self.global_step % self.config.trainer.val_freq == 0:
                    val_metrics = self._validate()
                    metrics.update(val_metrics)
                ...
                n_gpus = self.resource_pool_manager.get_n_gpus()
                metrics.update(compute_data_metrics(batch=batch, use_critic=self.use_critic))
                metrics.update(compute_throughout_metrics(batch=batch, timing_raw={}, n_gpus=n_gpus))
                logger.log(data=metrics, step=self.global_step)
